...
app.post('/generate-story', async (req, res) => {
  try {
    const { items = [], themeName, subThemeName, length, childName = '', childGender } = req.body;
    const targetWords = length === 'Short' ? 800 : length === 'Medium' ? 1200 : 2000;

    const protagonist = childName.trim() ? (childGender ? `${childName}, a ${childGender}` : childName) : null;
...
**TITLE RULE**:
Cute, funny, magical or endearing ORIGINAL title. NEVER include child's name, gender, theme, length, "adventure", "journey", "lesson".
...
Output ONLY valid JSON:
{
  ...
  "story": "Full story text..."
    ...
    const fallback = getFallbackStory(req.body.items || [], req.body.themeName, req.body.subThemeName, req.body.childName ? `${req.body.childName}, a ${req.body.childGender}` : null);
    res.status(500).json(fallback);
  }
});

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => console.log(`✅ BestieQuest Backend running on http://localhost:${PORT}`));
