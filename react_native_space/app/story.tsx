...
export default function StoryViewer() {
  ...
  const readAloud = () => {
    if (speaking) { Speech.stop(); setSpeaking(false); } else { Speech.speak(storyData.story, { rate: 0.9 }); setSpeaking(true); }
  };
  if (!storyData) return <Text className="text-center mt-20">Loading...</Text>;
  return (
    <ScrollView className="flex-1 bg-[#f8f4ff] p-6">
      <TouchableOpacity onPress={() => router.back()} className="mb-6"><Text className="text-purple text-xl">← Back</Text></TouchableOpacity>
      ...
      <View className="bg-white p-8 rounded-3xl mt-8 mb-12"><Text className="text-xl leading-relaxed text-gray-800">{storyData.story}</Text></View>
